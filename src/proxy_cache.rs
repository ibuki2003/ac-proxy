...
lazy_static! {
  static ref PATH_PATTERNS: Vec<Regex> = vec![
    Regex::new(r"^/users/\w+/history/json").unwrap(),
  ];
}
  ...
  match req.send().await {
    Ok(mut res) => {
      ...
      let body = match String::from_utf8(body.to_vec()) {
        Ok(body) => body,
        Err(_) => return HttpResponse::InternalServerError().body("500 Internal Server Error"),
      };
      let content_type = res.headers().get(http::header::CONTENT_TYPE).map(|s| { s.to_str().unwrap().to_string() });
      let date = res.headers().get(http::header::DATE).map(|s| { s.to_str().unwrap().to_string() });
      ...
      let res = HttpResponse::build(res.status())
        .insert_header((http::header::CONTENT_TYPE, content_type.unwrap_or(String::from("text/plain"))))
        .body(body);
      return res
    },
    Err(e) => {
...
async fn get_cache(path: &String, conn: &mut mysql_async::Conn) -> Option<HttpResponse> {
  "SELECT body, content_type, date FROM cache WHERE path = ?"
    ...
    .map(|(body, content_type, date)| {
      return HttpResponse::build(if body.is_some() { http::StatusCode::OK } else { http::StatusCode::NOT_FOUND })
...
async fn save_cache(path: &String, body: Option<&String>, content_type: &Option<String>, date: &Option<String>, conn: &mut mysql_async::Conn) -> Result<(), mysql_async::Error> {
